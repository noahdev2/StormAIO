...
namespace StormAIO.Champions
{
    internal class Chogath
    {
        ...
        public Chogath()
        {
            ...
            Q.SetSkillshot(0.5f, 200f, float.MaxValue, false, SkillshotType.Circle);
            W.SetSkillshot(0.5f, 60f, int.MaxValue, false, SkillshotType.Cone);
            ...
            {
                if (sender.IsEnemy  && champMenu["Q"].GetValue<MenuBool>("AQ") && Q.IsInRange(sender))
        ...
        {
            if (!champMenu["Q"].GetValue<MenuBool>("AQC")) return;
        ...
        private void Orbwalker_OnAction(object sender, OrbwalkerActionArgs args)
        {
            ...
            if (args.Type == OrbwalkerType.AfterAttack && args.Target is AITurretClient && champMenu["E"].GetValue<MenuBool>("ET") )
            {
                if (E.IsReady()) E.Cast();
            }
        }
        ...
        {
            if (champMenu["Drawing"].GetValue<MenuBool>("DrawQ") && Q.IsReady())
                Drawing.DrawCircle(Player.Position, Q.Range, Color.Plum);
            if (champMenu["Drawing"].GetValue<MenuBool>("DrawW") && W.IsReady())
                Drawing.DrawCircle(Player.Position, W.Range, Color.DarkCyan);
            if (champMenu["Drawing"].GetValue<MenuBool>("DrawR") && R.IsReady())
                Drawing.DrawCircle(Player.Position, R.Range + Player.BoundingRadius, Color.Firebrick);
        ...
        private void Combo()
        {
            ...
            if (R.IsReady() && target.DistanceToPlayer() < R.Range + Player.BoundingRadius && R.GetDamage(target) >
                ...
                champMenu["R"].GetValue<MenuBool>("RC"))
            {
                R.Cast(target);
            }
            else
            {
                if (champMenu["Q"].GetValue<MenuBool>("QC") && Player.Mana - 60 > Rmana) CastQ(target);
                if (champMenu["W"].GetValue<MenuBool>("WC") &&
                    Player.Mana - 70 + 10 * (W.Level - 1) > Rmana)
                    CastW(target);
                if (champMenu["E"].GetValue<MenuBool>("EC") && Player.Mana - 30 > Rmana) CastE(target);
            }
        }
        ...
        private void Harass()
        {
            var target = TargetSelector.GetTarget(Q.Range);
            if (target == null) return;
            if (champMenu["Q"].GetValue<MenuBool>("QH") &&
                champMenu["Q"].GetValue<MenuSlider>("QM").Value < Player.ManaPercent) CastQ(target);
            if (champMenu["W"].GetValue<MenuBool>("WH") && champMenu["W"].GetValue<MenuSlider>("WM").Value < Player.ManaPercent)
                CastW(target);
            if (champMenu["E"].GetValue<MenuBool>("EH") &&
        ...
        private void LaneClear()
        {
            ...
            bool useQ = champMenu["LaneClear"].GetValue<MenuBool>("Q");
            bool useW = champMenu["LaneClear"].GetValue<MenuBool>("W");
            bool useE = champMenu["LaneClear"].GetValue<MenuBool>("E");
        ...
        private void KillSteal()
        {
            var target = TargetSelector.GetTarget(Q.Range);
            if (target == null) return;
            if (target.Health < Q.GetDamage(target) && !target.IsInvulnerable && champMenu["KS"].GetValue<MenuBool>("Q"))
                CastQ(target);
            if (target.Health < W.GetDamage(target) && !target.IsInvulnerable && champMenu["KS"].GetValue<MenuBool>("W"))
                CastW(target);{}
        }
        ...
        {
            var targets = TargetSelector.GetTarget(R.Range + Player.BoundingRadius);
            if (targets == null) return;
            R.Cast(targets);
        }
