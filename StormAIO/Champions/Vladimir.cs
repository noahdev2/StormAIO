...
namespace StormAIO.Champions
{
    internal class Vladimir
    {
        ...
        private static void InitSpell()
        {
            ...
            E.SetSkillshot(0f, 600f, float.MaxValue, false, false, SkillshotType.Circle);
            E.SetCharged("VladimirE","VladimirE",50,590,1000f);
            R = new Spell(SpellSlot.R,625);
            R.SetSkillshot(0f,375f,2000f,false,SkillshotType.Circle);
        }
        ...
        {
            if (args.Type == OrbwalkerType.BeforeAttack && E.IsCharging)
            {
                args.Process = false;
            }
        }
        private void AIBaseClientOnOnProcessSpellCast(AIBaseClient sender, AIBaseClientProcessSpellCastEventArgs args)
        {
           ...
           if (sender is AITurretClient && args.Target.IsMe && Miscmenu.WBool)
           {
           ...
           {
               if (!W.IsReady() || !Miscmenu.WEvade) return;
               Whelper();
           }
        }
        ...
        { 
            ...
            if (HarassMenu.QLastHit) CastQMinion();
        ...
        {
            if (Q.IsReady() && JungleClearMenu.QBool)
            {
            ...
            if (E.IsReady() && JungleClearMenu.EBool)
            {
        ...
        {
            if (!Q.IsReady() || !LastHitMenu.QBool) return;
            var minions = GameObjects.GetMinions(Player.Position, Q.Range)
                .OrderBy(x=> x.DistanceToPlayer()).FirstOrDefault(x => Qdmg(x) > x.Health);
            if (minions == null) return;
            Q.Cast(minions);
        ...
        {
            if (Q.IsReady() && KillStealMenu.QBool)
            {
                var target = Q.GetTarget();
                if (target == null || Qdmg(target) < target.TrueHealth()) return;
                Q.Cast(target);
            }
            if (E.IsReady() && KillStealMenu.EBool)
            {
