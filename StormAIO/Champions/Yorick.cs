...
namespace StormAIO.Champions
{
    internal class Yorick
    {
        ...
        private static void InitSpell()
        {
            ...
            W.SetSkillshot(1f, 225, float.MaxValue, false, SkillshotType.Circle);
            E = new Spell(SpellSlot.E, 700);
            R = new Spell(SpellSlot.R, 600);
            R.SetTargetted(0.50f, float.MaxValue);
            E.SetSkillshot(0.33f, 100f, 500, false, SkillshotType.Line);
        }
        ...
        public Yorick()
        {
            ...
            {
                if (sender.IsEnemy && W.IsReady() && ChampMenu["Misc"].GetValue<MenuBool>("UseW") &&
                    W.IsInRange(sender))
                {
            ...
            {
                if (sender.IsEnemy && W.IsReady() && ChampMenu["Misc"].GetValue<MenuBool>("UseW2") &&
                    W.IsInRange(sender))
                {
                    var wpre = W.GetPrediction(sender);
        ...
        private void OrbwalkerOnOnAction(object sender, OrbwalkerActionArgs args)
        {
            ...
            {
                var target = TargetSelector.GetTarget(Player.GetRealAutoAttackRange() + 50);
                var RManaCost = !R.IsReady() ? 0 : 100;
                var QManaCost = Q.Level < 1 ? 0 : 25;
                if (ChampMenu["Q"].GetValue<MenuBool>("QC") && Player.Mana - QManaCost > RManaCost) CastQ(target);
            }
        }
        ...
        private void AIBaseClientOnOnProcessSpellCast(AIBaseClient sender, AIBaseClientProcessSpellCastEventArgs args)
        {
            ...
            if (sender.IsMe && args.Target is AITurretClient && HasfirstQ &&
                ChampMenu["StructureClear"].GetValue<MenuBool>("QT") && Orbwalker.ActiveMode == OrbwalkerMode.LaneClear) Q.Cast();
        }

        private static void Drawing_OnDraw(EventArgs args)
        {
            if (ChampMenu["Drawing"].GetValue<MenuBool>("DrawQ") && Q.IsReady())
                Drawing.DrawCircle(Player.Position, Player.GetRealAutoAttackRange() + 50, Color.DarkCyan);
            if (ChampMenu["Drawing"].GetValue<MenuBool>("DrawW") && W.IsReady())
                Drawing.DrawCircle(Player.Position, W.Range, Color.DarkCyan);
            if (ChampMenu["Drawing"].GetValue<MenuBool>("DrawE") && E.IsReady())
        ...
        private static void Combo()
        {
            ...
            if (ChampMenu["R"].GetValue<MenuBool>("R")) CastR(target);
            if (ChampMenu["Q"].GetValue<MenuBool>("QC") && Player.Mana - QManaCost > RManaCost) CastQ2(target);
            if (ChampMenu["W"].GetValue<MenuBool>("WC") && Player.Mana - WManaCost > RManaCost) CastW(target);
            if (ChampMenu["E"].GetValue<MenuBool>("EC") && Player.Mana - EManaCost > RManaCost) CastE(target);
        }
        ...
        private static void Harass()
        {
            ...
            if (ChampMenu["W"].GetValue<MenuBool>("WH") &&
        ...
        private static void KillSteal()
        {
            ...
            if (target.TrueHealth() < E.GetDamage(target)  &&
                ChampMenu["KS"].GetValue<MenuBool>("E")) CastE(target);
        }
