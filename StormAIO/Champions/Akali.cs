...
namespace StormAIO.Champions
{
    internal class Akali
    {
        ...
        private static void Drawing_OnDraw(EventArgs args)
        {
            ...
            if (ChampMenu["Drawing"].GetValue<MenuBool>("DrawR2") && R.IsReady())
        ...
        private static void Combo()
        {
            ...
            if (ChampMenu["E"].GetValue<MenuBool>("E2")) CastE2();
            var target = TargetSelector.GetTarget(E.Range);
            if (target == null) return;
            if (ChampMenu["R"].GetValue<MenuBool>("R") && R3) CastR();
            if (ChampMenu["R"].GetValue<MenuBool>("R2") && R3) CastR2();
            if (ChampMenu["Q"].GetValue<MenuBool>("QC")) CastQ();
            if (ChampMenu["W"].GetValue<MenuBool>("WC") &&
                Player.Mana < ChampMenu["W"].GetValue<MenuSlider>("WU").Value) CastW(target);
            if (ChampMenu["E"].GetValue<MenuBool>("E1")) CastE();
        ...
        private static void Harass()
        {
            ...
            if (target == null) return;
            if (ChampMenu["Q"].GetValue<MenuBool>("QH")) CastQ();
        }
        ...
        private static void KillSteal()
        {
            ...
            if (e2target != null)
                if (e2target.TrueHealth() < Edmg(e2target) + passivedmg(e2target) + Q.GetDamage(e2target) &&
                    ChampMenu["KS"].GetValue<MenuBool>("E2"))
                    CastE2();

            var target = TargetSelector.GetTarget(E.Range);
            if (target == null) return;
            if (target.TrueHealth() < Q.GetDamage(target) && ChampMenu["KS"].GetValue<MenuBool>("Q")) CastQ();
            if (target.TrueHealth() < Edmg(target) && !target.HasBuffOfType(BuffType.SpellShield) &&
                ChampMenu["KS"].GetValue<MenuBool>("E1")) CastE();

            if (target.TrueHealth() < bothRdmg(target) && ChampMenu["KS"].GetValue<MenuBool>("R1")) CastR();
            if (target.TrueHealth() < bothRdmg(target, 1) && ChampMenu["KS"].GetValue<MenuBool>("R2")) CastR2();
            if (target.TrueHealth() < bothRdmg(target, 1) && ChampMenu["KS"].GetValue<MenuBool>("R2")) CastR2();
            var gunbladedmg =
                Player.CalculateMagicDamage(target, 170 + 4.588 * Player.Level + Player.TotalMagicalDamage * 0.3);
            if (Player.CanUseItem((int) ItemId.Hextech_Gunblade) &&
                target.Distance(Player) <= 700 && target.TrueHealth() < gunbladedmg &&
                ChampMenu["KS"].GetValue<MenuBool>("G")) Player.UseItem(3146, target);
            if (Player.CanUseItem((int) ItemId.Bilgewater_Cutlass) && target.Distance(Player) <= 550 &&
                target.TrueHealth() < Player.CalculateMagicDamage(target, 100) &&
                ChampMenu["KS"].GetValue<MenuBool>("G")) Player.UseItem((int) ItemId.Bilgewater_Cutlass, target);
        }
