...
namespace StormAIO.Champions
{
    internal class Kowmaw
    {
        ...
        private static class ComboMenu
        {
            ...
            public static readonly MenuSliderButton RHpBool     = new MenuSliderButton("rhp", "^ Only if target HP <= X%", 40);
            public static readonly MenuSliderButton RStacksBool = new MenuSliderButton("rstacks", "^ Max Stacks", 3, 1, 9);

            public static readonly MenuBool LethalTempoBool = new MenuBool("lethaltempo", "Block Q/E/R Spells if Lethal Tempo is Active");
        ...
        private static class HarassMenu
        {
            ...
            public static readonly MenuSliderButton RBool       = new MenuSliderButton("r", "Use R | If Mama >= x%", 50);
        ...
        public Kowmaw()
        {
            ...
            if (R.Level != 0) R.Range = 1300f + 250 * (R.Level - 1);
            if (W.Level != 0) W.Range = 630f + 20 * (W.Level - 1) + Player.GetRealAutoAttackRange();
            new DrawText("Simi R Key",ComboMenu.RSemiAutoKeyBind.Key.ToString(),ComboMenu.RSemiAutoKeyBind,Color.GreenYellow,Color.Red,123,132);
        }
        ...
        private void TeleportOnOnTeleport(AIBaseClient Sender, Teleport.TeleportEventArgs args)
        {
            ...
            if (!AutomaticMenu.EOnTPBool.Enabled)
            {
                return;
            }
        ...
        {
            var Minions = GameObjects.GetMinions(Player.Position, E.Range).Where(x=> x.IsEnemy).OrderBy(x => x.DistanceToPlayer())
                .FirstOrDefault();
            if (Minions != null && Q.IsInRange(Minions) && Player.ManaPercent > LaneClearMenu.QSliderBool.ActiveValue && LaneClearMenu.QSliderBool.Enabled)
            {
                if (!Q.IsReady()) return;
                if (Minions.Health < Q.GetDamage(Minions)) Q.Cast(Minions);
            }

            if (Minions != null && E.IsReady() && Player.ManaPercent > LaneClearMenu.ESliderBool.ActiveValue && LaneClearMenu.ESliderBool.Enabled)
            {
                var allminions = GameObjects.GetMinions(Player.Position,E.Range);
                var eline = E.GetLineFarmLocation(allminions, E.Width);
                if (allminions != null && eline.MinionsHit >= 2)
                {
        ...
        private static void JungleClear()
        {
            var jungle = GameObjects.GetJungles(Player.Position, E.Range).OrderBy(x => x.DistanceToPlayer())
                .FirstOrDefault();
            if (jungle != null && Q.IsInRange(jungle) && Player.ManaPercent > JungleClearMenu.QSliderBool.ActiveValue && JungleClearMenu.QSliderBool.Enabled)
            {
                if (!Q.IsReady()) return;
                Q.Cast(jungle);
            }

            if (jungle != null && E.IsReady() && Player.ManaPercent > JungleClearMenu.ESliderBool.ActiveValue && JungleClearMenu.ESliderBool.Enabled)
            {
                E.Cast(jungle);
            }
        }
        ...
        private static void KillSteal()
        {
            ...
            if (rtarget != null && R.IsReady() && KillStealMenu.RBool)
            {
                if (rtarget.TrueHealth() < R.GetDamage(rtarget))
                {
                   var rpre = R.GetPrediction(rtarget);
                   if (rpre.Hitchance >= HitChance.High)
                   {
            ...
            if (qtarget != null && Q.IsReady() && KillStealMenu.QBool)
            {
            ...
            if (etarget != null && E.IsReady() && KillStealMenu.EBool)
            {
        ...
        private static void CastE()
        {
            ...
            if (ComboMenu.EWBool.Enabled && HasWActive)
            {
                return;
            }

            if (HasLTActive && ComboMenu.LethalTempoBool.Enabled)
            {
                return;
            }
        ...
        private static void CastR()
        {
            ...
            if (GetRStacks > ComboMenu.RStacksBool.Value && ComboMenu.RStacksBool.Enabled)
            {
                return;
            }
            ...
            if (ComboMenu.RRangeBool.Enabled && target.DistanceToPlayer() <= Player.GetRealAutoAttackRange())
            {
                return;
            }
        ...
        {
            if (!AutomaticMenu.EOnCCBool.Enabled)
            {
                return;
            }
        ...
        public static void ROnImmobile()
        {
            if (!AutomaticMenu.ROnCCBool.Enabled)
            {
                return;
            }
        ...
        public static bool HasWActive => Player.HasBuff("KogMawBioArcaneBarrage");

        public static bool HasLTActive =>  Player.HasBuff("ASSETS/Perks/Styles/Precision/LethalTempo/LethalTempoEmpowered.lua");

        public static int GetRStacks => Player.GetBuffCount("kogmawlivingartillerycost");
        public static AIHeroClient Lulu { get; set; }

        public static bool WaitForLulu()
        {
            if (!ComboMenu.Lulu.Enabled)
            {
                return false;
            }
