...
namespace MightyAio.Champions
{
    internal class Yorick
    {
        ...
        {
            _menu = new Menu("MightyYorick", "Mighty Yorick", true);
        ...
        public Yorick()
        {
            ...
            _w.SetSkillshot(1f,225,float.MaxValue,false,SkillshotType.Circle);
            _e = new Spell(SpellSlot.E, 700);
            _r = new Spell(SpellSlot.R, 600);
            _r.SetTargetted(0.50f, float.MaxValue);
            _e.SetSkillshot(0.33f, 100f, 500, false, SkillshotType.Line);
            CreateMenu();
            ...
            {
                if (sender.IsEnemy && _w.IsReady() && _menu["Misc"].GetValue<MenuBool>("UseW") && _w.IsInRange(sender))
                {
            ...
            {
                if (sender.IsEnemy && _w.IsReady() && _menu["Misc"].GetValue<MenuBool>("UseW2")  && _w.IsInRange(sender))
                {
                    var wpre = _w.GetPrediction(sender);
        ...
        private void OrbwalkerOnOnAction(object sender, OrbwalkerActionArgs args)
        {
            ...
            if (args.Type == OrbwalkerType.AfterAttack && Orbwalker.ActiveMode == OrbwalkerMode.Combo)
            {
                ...
                if (_menu["Q"].GetValue<MenuBool>("QC") && Player.Mana - QManaCost > RManaCost)
                {
                    CastQ(target);
                }
            }
        ...
        private void AIBaseClientOnOnProcessSpellCast(AIBaseClient sender, AIBaseClientProcessSpellCastEventArgs args)
        {
            ...
            if (sender.IsMe && args.Target is AITurretClient && HasfirstQ && _menu["LaneClear"].GetValue<MenuBool>("QT"))
            {
                _q.Cast();
            }
        }
        ...
        {
            if (_menu["Drawing"].GetValue<MenuBool>("DrawQ") && _q.IsReady())
        ...
        private static void Combo()
        {
            ...
            if (_menu["Q"].GetValue<MenuBool>("QC") && Player.Mana - QManaCost > RManaCost) { CastQ2(target); }
            if (_menu["W"].GetValue<MenuBool>("WC") && Player.Mana - WManaCost > RManaCost) CastW(target);
            if (_menu["E"].GetValue<MenuBool>("EC") && Player.Mana - EManaCost > RManaCost) CastE(target);
        }
