...
namespace MightyAio.Champions
{
    internal class Chogath
    {
        ...
        {
            _menu = new Menu("MightyChogath", "Mighty Chogath", true);
        ...
        public Chogath()
        {
            ...
            {
                if (sender.IsEnemy  && _menu["Q"].GetValue<MenuBool>("AQ") && _q.IsInRange(sender))
        ...
        {
            if (_menu["Drawing"].GetValue<MenuBool>("DrawQ"))
            ...
            if (_menu["Drawing"].GetValue<MenuBool>("DrawR"))
                Drawing.DrawCircle(Player.Position, _r.Range + Player.BoundingRadius, Color.Firebrick);
        ...
        private void Combo()
        {
            ...
            // use Stone Plate 
            if (_items.GetValue<MenuBool>("Stone") && _r.IsReady() && Player.CanUseItem(3193) &&
            ...
            if (_r.IsReady() && target.DistanceToPlayer() < _r.Range + Player.BoundingRadius && _r.GetDamage(target) >
                ...
                !target.HasBuffOfType(BuffType.SpellShield) && _menu["R"].GetValue<MenuBool>(target.CharacterName) &&
                _menu["R"].GetValue<MenuBool>("RC"))
            {
                _r.Cast(target);
            }
            else
            {
                if (_menu["Q"].GetValue<MenuBool>("QC") && Player.Mana - 60 > Rmana) CastQ(target);
                if (_menu["W"].GetValue<MenuBool>("WC") &&
                    Player.Mana - 70 + 10 * (_w.Level - 1) > Rmana)
                    CastW(target);
                if (_menu["E"].GetValue<MenuBool>("EC") && Player.Mana - 30 > Rmana) CastE(target);
            }
        }
        ...
        private void Harass()
        {
            ...
            if (_menu["W"].GetValue<MenuBool>("WH") && _menu["W"].GetValue<MenuSlider>("WM").Value < Player.ManaPercent)
                CastW(target);
        ...
        private void KillSteal()
        {
            var target = TargetSelector.GetTarget(_q.Range);
            if (target == null) return;
            if (target.Health < _q.GetDamage(target) && !target.IsInvulnerable && _menu["KS"].GetValue<MenuBool>("Q"))
                CastQ(target);
