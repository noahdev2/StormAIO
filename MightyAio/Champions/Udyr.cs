    ...
    internal class Udyr
    {
        ...
        private void AIBaseClientOnOnProcessSpellCast(AIBaseClient sender, AIBaseClientProcessSpellCastEventArgs args)
        {
            if (sender.IsEnemy && sender.IsValidTarget(800))
                if (_e.IsReady() &&
                    Player.Distance(sender) <= 800 && _menu["E"].GetValue<MenuBool>("EI"))
                {
                    if (!isInEStance)
                        _e.Cast();
                    else
        ...
        private void InterrupterOnOnInterrupterSpell(AIHeroClient sender, Interrupter.InterruptSpellArgs args)
        {
            if (sender.IsEnemy && sender.IsValidTarget(800))
                ...
                {
                    if (!isInEStance && _menu["E"].GetValue<MenuBool>("EI"))
                        _e.Cast();
                    else
                        return;
                }

            if (sender.IsEnemy && sender.IsCastingImporantSpell() && _menu["W"].GetValue<MenuBool>("WS")) _w.Cast();
        }

        private void GapcloserOnOnGapcloser(AIHeroClient sender, Gapcloser.GapcloserArgs args)
        {
            if (sender.IsEnemy && _e.IsReady() && _menu["E"].GetValue<MenuBool>("EG") &&
        ...
        private void Combo()
        {
            ...
            if (_e.IsReady() && _menu["E"].GetValue<MenuBool>("EC"))
            {
            ...
            if (_w.IsReady() && Player.HealthPercent <= _menu["W"].GetValue<MenuSlider>("WHC").Value &&
                _menu["W"].GetValue<MenuBool>("WC") && Player.Distance(target) <= _w.Range &&
                target.HasBuff("udyrbearstuncheck")) _w.Cast();
            if (_q.IsReady() && _menu["Q"].GetValue<MenuBool>("QC") && Player.Distance(target) <= _q.Range &&
                target.HasBuff("udyrbearstuncheck")) _q.Cast();
            if (_r.IsReady() && _menu["R"].GetValue<MenuBool>("RC") && Player.Distance(target) <= _r.Range &&
        ...
        private void Harass()
        {
            var target = TargetSelector.GetTarget(900);
            if (target == null) return;
            if (_e.IsReady() && _menu["E"].GetValue<MenuBool>("EH") &&
            ...
            if (_q.IsReady() && _menu["Q"].GetValue<MenuSlider>("QM").Value > Player.ManaPercent &&
                _menu["Q"].GetValue<MenuBool>("QH") && Player.Distance(target) <= _q.Range &&
                target.HasBuff("udyrbearstuncheck")) _q.Cast();
            if (_r.IsReady() && _menu["R"].GetValue<MenuSlider>("RM").Value > Player.ManaPercent &&
                _menu["R"].GetValue<MenuBool>("RH") && Player.Distance(target) <= _r.Range &&
                target.HasBuff("udyrbearstuncheck")) _r.Cast();
        }
        ...
        private void JungleClear()
        {
            ...
            if (_menu["LaneClear"].GetValue<MenuBool>("P"))
            {
            ...
            if (!isInRStance && _r.IsReady() && _menu["LaneClear"].GetValue<MenuBool>("R"))
            {
            ...
            if (_menu["LaneClear"].GetValue<MenuBool>("Q") && _q.IsReady())
                _q.Cast();

            else if (_menu["LaneClear"].GetValue<MenuBool>("W") && !isInWStance && _w.IsReady() &&
        ...
        private void Feel()
        {
            ...
            var Feelbm = _menu["E"].GetValue<MenuBool>("FeelB");
            if (_e.IsReady())
            {
                _chatsent = 0;
                _e.Cast();
