...
namespace MightyAio.Champions
{
    internal class Fizz
    {
        ...
        public Fizz()
        {
            ...
            R.SetSkillshot(0.25f, 80, 1300, false, SkillshotType.Line);
            RC.SetSkillshot(0.25f, 80, 1300, false, SkillshotType.Line);
            RG.SetSkillshot(0.25f, 80, 1300, false, SkillshotType.Line);
            RG.SetSkillshot(0.25f, 80, 1300, false, SkillshotType.Line);
        ...
        private static void DrawingOnOnDraw(EventArgs args)
        {
            ...
            if (drawQ && Q.IsReady()) Drawing.DrawCircle(p, Q.Range, Color.DarkCyan);

            if (drawE && E.IsReady()) Drawing.DrawCircle(p, E.Range, Color.Red);

            if (drawR && R.IsReady()) Drawing.DrawCircle(p, R.Range, Color.DarkCyan);
            ...
            AIBaseClientProcessSpellCastEventArgs args)
        {
            if (sender is AITurretClient && args.Target.IsMe && E.IsReady() &&
                Menu["Misc"].GetValue<MenuBool>("UseETower")) E.Cast(Game.CursorPos);
        ...
        private static void GameOnOnUpdate(EventArgs args)
        {
            ...
            if (item != "none")
                switch (item)
                {
                    ...
                    case "The Dark Seal":
                    {
                        if (time < 1 && Player.InShop())
                        {
                            if (gold >= 500 && !Player.HasItem(ItemId.The_Dark_Seal))
                                Player.BuyItem(ItemId.The_Dark_Seal);
            ...
            var useonkill = Menu["emotes"].GetValue<MenuBool>("Kill");
        ...
        private static void DoLastHit()
        {
            ...
            var usew = Menu["lasthit"].GetValue<MenuBool>("UseWlasthit");
            var ks = Menu["lasthit"].GetValue<MenuBool>("UseQWlasthit");

            if (!Player.IsMoving) Orbwalker.MovementState = true;
        ...
        private static void Feel()
        {
            ...
            var qto = TargetSelector.GetTarget(Q.Range);
            var UseEtofeel = Menu["Feel"].GetValue<MenuBool>("UseEtofeel");
            var UseQtofeel = Menu["Feel"].GetValue<MenuBool>("UseQtofeel");
        ...
        private static void DoLaneClear()
        {
            var allMinions = GameObjects.GetMinions(ObjectManager.Player.Position, Q.Range);
            var allJgl = GameObjects.GetJungles(ObjectManager.Player.Position, Q.Range);
            var useq = Menu["laneclear"].GetValue<MenuBool>("UseQ");
            var usew = Menu["laneclear"].GetValue<MenuBool>("UseW");
            var usee = Menu["laneclear"].GetValue<MenuBool>("UseE");
        ...
        {
            var target = TargetSelector.GetTarget(R.Range);
            var allin = Menu["Combo"].GetValue<MenuBool>("UseAllinCombo");
            var comboE = Menu["Combo"].GetValue<MenuBool>("UseEEQWCombo");
            var QWE = Menu["Combo"].GetValue<MenuBool>("UseQWECombo");
            var QW = Menu["Combo"].GetValue<MenuBool>("UseQWCombo");
            // var autowauto = Menu["Combo"].GetValue<MenuBool>("autowauto");
            var Qability = Menu["Combo"].GetValue<MenuBool>("UseQCombo");
            var Wability = Menu["Combo"].GetValue<MenuBool>("UseWCombo");
            var Eability = Menu["Combo"].GetValue<MenuBool>("UseECombo");
            var usegun = Menu["Items"].GetValue<MenuBool>("GunbladeCombo");
            var useHP = Menu["Items"].GetValue<MenuBool>("HPKS2");
            var minmama = Menu["Combo"].GetValue<MenuSlider>("OnlyUse").Value;
            var Usespellbinder = Menu["Items"].GetValue<MenuBool>("Use9");
            ...
            else if (target.HasBuff("fizzrcling") && comboE && Q.IsReady() && W.IsReady() && E.IsReady())
            {
                ...
                {
                    if (Player.HasItem((int) ItemId.Will_of_the_Ancients) ||
                        Player.HasItem((int) ItemId.Hextech_Gunblade))
                        if (useHP || usegun)
                        {
                            if (Player.CanUseItem((int) ItemId.Hextech_Gunblade) && target.Distance(Player) < 700)
                                Player.UseItem(3146, target);
                            if (Player.CanUseItem((int) ItemId.Will_of_the_Ancients))
                                Player.UseItem(3152, target.Position);
                        }
        ...
        {
            var target = TargetSelector.GetTarget(R.Range);
            var useonrhit = Menu["emotes"].GetValue<MenuBool>("Kill");
            ...
            var ksr = Menu["KillSteal"].GetValue<MenuBool>("UseR");
            var ksqwE = Menu["KillSteal"].GetValue<MenuBool>("UseQWE");
            var ksqw = Menu["KillSteal"].GetValue<MenuBool>("UseQW");
            var ksE = Menu["KillSteal"].GetValue<MenuBool>("UseE");
            var ksflash = Menu["KillSteal"].GetValue<MenuBool>("UseFlash");
            var ksq = Menu["KillSteal"].GetValue<MenuBool>("UseQ");
            var ksgun = Menu["Items"].GetValue<MenuBool>("GunbladeKS");
            var HPKS = Menu["Items"].GetValue<MenuBool>("HPKS");

            if (Player.HasItem((int) ItemId.Hextech_Gunblade) && Player.CanUseItem((int) ItemId.Hextech_Gunblade) &&
                target.Distance(Player) < 700 && target.Health < gunbladedmg && ksgun) Player.UseItem(3146, target);
            if (Player.HasItem((int) ItemId.Will_of_the_Ancients) &&
                Player.CanUseItem((int) ItemId.Will_of_the_Ancients) && target.Distance(Player) < 900 &&
                target.Health < Hextechdmg && HPKS) Player.UseItem(3152, target.Position);
            if (rg == 1)
        ...
        {
            var QWE = Menu["Harass"].GetValue<MenuBool>("UseQWECombo");
            var QW = Menu["Harass"].GetValue<MenuBool>("UseQWCombo");
            // var autowauto = Menu["Combo"].GetValue<MenuBool>("autowauto");
            var Qability = Menu["Harass"].GetValue<MenuBool>("UseQCombo");
            var Wability = Menu["Harass"].GetValue<MenuBool>("UseWCombo");
            var Eability = Menu["Harass"].GetValue<MenuBool>("UseECombo");
            var target = TargetSelector.GetTarget(Q.Range + E.Range);
        ...
        {
            var auto = Menu["autolevel"].GetValue<MenuBool>("autolevelu");
            if (!auto) return;
