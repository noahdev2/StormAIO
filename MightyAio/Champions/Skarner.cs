...
namespace MightyAio.Champions
{
    internal class Skarner
    {
        ...
        {
            _menu = new Menu("MightySkarner", "Mighty Skarner", true);
        ...
        public Skarner()
        {
            ...
            _e.SetSkillshot(0.25f, 70f, 1500, false, SkillshotType.Line);
            CreateMenu();
        ...
        private void AIBaseClientOnOnProcessSpellCast(AIBaseClient sender, AIBaseClientProcessSpellCastEventArgs args)
        {
            ...
            if (AutoW && _w.IsReady() && args.Target.IsMe && sender.IsMonster && args.Target.IsMe) // For jgl Clear
                _w.Cast();
        }
        ...
        private static void Combo()
        {
            ...
            if (_menu["Q"].GetValue<MenuBool>("QC") && Player.Mana - QManaCost > RManaCost) CastQ(target);
            if (_menu["E"].GetValue<MenuBool>("EC") && Player.Mana - 55 > RManaCost) CastE(target);
            if (haspassivebuff(target) && target.DistanceToPlayer() > 300 &&
                _menu["W"].GetValue<MenuBool>("WC")) _w.Cast();
        }
        ...
        private static void CastR(AIHeroClient target)
        {
            ...
            if (_r.IsInRange(target) && !target.HasBuffOfType(BuffType.SpellShield) &&
                _Rmenu.GetValue<MenuBool>(target.CharacterName) && target.IsUnderEnemyTurret()) _r.Cast(target);
            if (_r.IsInRange(target) && !target.HasBuffOfType(BuffType.SpellShield) &&
                _Rmenu.GetValue<MenuBool>(target.CharacterName) && !haspassivebuff(target)) _r.Cast(target);
        }
        ...
        private static void FlashR()
        {
            ...
            if (!_r.IsReady() || target == null || Player.Spellbook.CanUseSpell(flash) != SpellState.Ready ||
                !_FlashR.GetValue<MenuBool>(target.CharacterName))
                return;
