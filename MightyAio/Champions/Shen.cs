...
namespace MightyAio.Champions
{
    internal class Shen
    {
        ...
        {
            _menu = new Menu("MightyShen", "Mighty Shen", true); 
        ...
        public Shen()
        {
            ...
            _e.SetSkillshot(0f, 50, 500, false, SkillshotType.Line);
            ...
            {
                if (_e.IsReady() && sender.IsValidTarget(_e.Range) && _menu["E"].GetValue<MenuBool>("EG"))
                    _e.Cast(args.EndPosition);
            };
            Interrupter.OnInterrupterSpell += (sender, args) =>
            {
                if (sender.IsEnemy && _e.IsReady() && sender.IsValidTarget(_e.Range) && _menu["E"].GetValue<MenuBool>("EI"))
                    _e.Cast(sender);
            };
        ...
        private static void Game_OnUpdate(EventArgs args)
        {
           ...
           if (_menu["R"].GetValue<MenuBool>("R")) AutoR();
        }
        ...
        private static void Combo()
        {
            ...
            if (_e.IsReady() && _e.IsInRange(target) && _menu["E"].GetValue<MenuBool>("EC")) _e.Cast(predepos);
            if (_q.IsReady() && _q.IsInRange(target) && _menu["Q"].GetValue<MenuBool>("QC")) _q.Cast();
            if (_w.IsReady() && Player.Distance(ShenWpos()) < _w.Range && target.Distance(ShenWpos()) < _w.Range && _menu["W"].GetValue<MenuBool>("WC")) _w.Cast();
        }
        ...
        private static void Harass()
        {
            ...
            if (target == null) return;
            var predepos = _e.GetPrediction(target).CastPosition;
            if (_e.IsReady() && _e.IsInRange(target) && _menu["E"].GetValue<MenuBool>("EH")) _e.Cast(predepos);
            if (_q.IsReady() && _q.IsInRange(target) && _menu["Q"].GetValue<MenuBool>("QH")) _q.Cast();
            if (_w.IsReady() && Player.Distance(ShenWpos()) < _w.Range && target.Distance(ShenWpos()) < _w.Range && _menu["W"].GetValue<MenuBool>("WH")) _w.Cast();
        }

        private static void LaneClear()
        {
            if (!_menu["LaneClear"].GetValue<MenuBool>("Q")) return;
        ...
        {
            if (!_menu["Q"].GetValue<MenuBool>("QL")) return;
            var minion = GameObjects.GetMinions(Player.Position, _q.Range).OrderBy(x => x.DistanceToPlayer())
                .FirstOrDefault();
            Game.Print(_q.GetDamage(minion));
        ...
        private static void AutoR()
        {
            ...
            foreach (var ally in allies.Where(x => x.IsValidTarget(_r.Range) && x.CharacterName != "Shen" && _menu["R"].GetValue<MenuBool>(x.CharacterName)))
                {
        ...
        private static void KillSteal()
        {
            ...
            var predepos = _e.GetPrediction(target).CastPosition;
            if (_e.IsReady() && _e.IsInRange(target) && _menu["KS"].GetValue<MenuBool>("E")) _e.Cast(predepos);
        }
